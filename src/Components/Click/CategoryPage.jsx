...
const CategoryPage = () => {
    ...
    useEffect(() => {
        ...
        const filters = catagory.filter((product)=>product.id == id );
       setProduct(filters[0])
    // console.log(filters)
  ...
  return (
    <div>
     ...
     <div className='border border-yellow-50 bg-teal-200 flex'>
                        <h1 className='text-3xl font-bold my-8 mx-8'>{product?.heading1}</h1>
      ...
      <nav className='flex mt-12'>
        <main className='flex'>
            ...
            <Link><img src={product?.img1} alt="" className=''/></Link>
          
            <Link><h1 className='text-5xl ms-16 mt-28'><SlArrowRight/></h1></Link>
        </main>
       {/* <div className='mt-96 '> <img src="Images/j1.jpg" alt="" /></div> */}
        <main>
            <h1 className='font-bold text-2xl mb-6 ms-12'>{product?.heading2} </h1>
            <p className='ms-12  mb-6'>{product?.heading2}</p>
            <hr />
